...
func InitApp(ctx context.Context) (*App, error) {
	...
	executorRegistry := stream.NewMemoryExecutorRegistry()
	executorOrchestrator := orchestrator.NewExecutorOrchestrator(agentBuilder, agentFactory, taskRegistry, executorRegistry)
	appRepository := app.NewAppRepository(transactionManager)
	eventStore := provider.ProvideRedisEventStore(client)
	chatAppService := service3.NewChatAppService(executorOrchestrator, appRepository, eventStore, executorRegistry)
